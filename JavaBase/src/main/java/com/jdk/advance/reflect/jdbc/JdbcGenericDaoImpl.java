...
public class JdbcGenericDaoImpl<T> implements GenericDao<T> {
   ...
   /* @Override
    ...
    public List<T> getALL(Class<T> clazz) throws Exception {
        ...
        String tableName = getTableName(clazz).toLowerCase();//获取表明

        Field[] fields = clazz.getDeclaredFields();//获取属性，是全包名的
        List<String> collect = Arrays.stream(fields).map(Field::getName).collect(Collectors.toList());//解析获取属性名
        String fieldNames = Joiner.on(",").join(collect);//拼接属性sql

        String sql = "select " + fieldNames + " from " + tableName;
        System.out.println(sql);//select id,name,pwd from user
    ...
    public List<T> get() throws Exception {
        List<T> list = Lists.newArrayList();
        Connection connection = JdbcDaoHelper.getConnection();
        String tableName = clazz.getSimpleName().toLowerCase();//获取表明
   ...
   /* @Override
    ...
    private void initObject(T t, Field[] fields, ResultSet rs) throws Exception {
        for (Field field : fields) {
            ...
            Class<?> clazzField = field.getType();//属性类型
            ...
            pd.getWriteMethod().invoke(t, paramVal);//调用set方法。Write为set，read为get
        }
    }
